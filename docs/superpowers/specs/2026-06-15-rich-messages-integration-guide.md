...
- **Status (landed):** worker core reworked to ONE rich message — `remove_queued_sticker` → `send_draw_placeholder(compose_draw_progress("рисую…",0,None,&[]))` → `generate`/`edit` → `publish_draw_images` (uploader) → `edit_draw_message(compose_gallery(...))`; NSFW/failure edit the same message to a notice. `ImageJobEffects` slimmed to 4 methods (`remove_queued_sticker`, `send_draw_placeholder`, `edit_draw_message`, `publish_draw_images`); `TelegramImageJobEffects` dropped its `Ephemeral` generic and now holds `rich: Arc<dyn crate::rich::RichSender>`. **Deferred:** the dynamic queue-wait countdown (state 1) needs an enqueue-side placeholder + a poller editing it as the queue drains; `rich::compose_draw_waiting(ahead)` already exists for it.
- `TelegramImageJobEffects` (struct/`new`): holds `rich: Arc<dyn crate::rich::RichSender>`; constructor arg order `new(queued_stickers, telegram, rich)`.
