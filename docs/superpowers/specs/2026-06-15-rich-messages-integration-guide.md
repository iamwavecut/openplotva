...
- **Status (landed): two-message flow.** (1) **Enqueue** (`dialog_tools` `schedule_image`/`schedule_image_edit`): post `compose_draw_waiting(ahead)` via the adapter's `RichSender` and persist the id with `queue.set_queue_position_message_id(job_id, …)`. (2) **Live update**: each worker dequeue calls `refresh_image_queue_positions` — re-renders the queue-position message of every still-pending image job to its new `ahead` (event-driven, no idle poller). (3) **Draw start**: the worker reads `job_queue_position_message_id`, `delete_message`s it, then `send_draw_placeholder` posts a FRESH "рисую…" message (so the result surfaces near the end of the chat) → `publish_draw_images` → `edit_draw_message(compose_gallery(...))`; NSFW/failure edit that message to a notice.
- `ImageJobEffects` is 4 methods: `delete_message`, `send_draw_placeholder`, `edit_draw_message`, `publish_draw_images`. `TelegramImageJobEffects<Sender>` dropped both the `Ephemeral` and `Queued` generics; `new(telegram, rich)` with `rich: Arc<dyn crate::rich::RichSender>`. New additive taskman API: `pending_image_queue_entries`, `set_queue_position_message_id`, `job_queue_position_message_id`, `PendingImageQueueEntry`. The vestigial `QueuedStickerStore`/queued-sticker path was removed from the app side.
- Replace the sticker + placeholder-media-group + per-slot `editMessageMedia` flow with ONE rich message that moves through states, each an in-place edit of the same message:
  1. **Queue-wait** — "⏳ ожидаю в очереди: N заказов впереди", **dynamically updated** as the queue drains (poll the queue depth ahead of this job and `edit()` the placeholder when N changes). Use the existing `TaskQueueRecord.queue_position_message_id` / taskman `image_queue_status` mechanism + a small updater loop (throttled ~1/sec, honoring `retry_after`).
  2. **Drawing** — "✨ рисую…" (with `N из M` once counts are known).
  3. **Final** — `compose_gallery(&urls, &caption_html, prompt_spoiler)`.
  Add a `rich::compose_draw_waiting(ahead: usize)` composer for state 1. Upload each image (bytes/url) via `self.rich.uploader()`. Private chats may `draft()`; groups `send()`+`edit()`. Persist the one progress message id on `TaskQueueRecord.result_message_id`/`queue_position_message_id` for edits. NOTE: generation is currently a single blocking call, so true per-image progress needs incremental generation; until then state 2 is one "рисую…" edit, but the queue-wait countdown (state 1) is genuinely dynamic.
- Delete `send_initial_placeholders`/`replace_placeholder_image`/`delete_placeholder_image`/`send_generated_images`/`fill_image_placeholders`/`cleanup_image_placeholders` and their tests; drop the drawing sticker.
- Composition root ~9183/9215/9244 (vip gen, vip edit, regular): 4th arg `Arc::clone(&rich_messenger)`.
- Contracts: caption template (author/VIP crown/#nsfw/support, ≤1024 visible), queue routing, NSFW Forbidden short-circuit, aspect ratio. 1..N images via slideshow.
