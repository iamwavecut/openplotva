...
- `TelegramImageJobEffects` (struct/`new` ~1127): add `rich: Arc<crate::rich::RichMessenger>`.
- Replace the sticker + placeholder-media-group + per-slot `editMessageMedia` flow with ONE rich message: send `compose_draw_progress("ожидаю очереди",0,None,&[])` → on each finished image `edit()` with `compose_draw_progress("рисую…", done, Some(total), &urls)` → final `compose_gallery(&urls, &caption_html, prompt_spoiler)`. Upload each image (bytes/url) via `self.rich.uploader()`. Private chats may `draft()`; groups `send()`+`edit()`. Persist the one progress message id on `TaskQueueRecord.result_message_id` for edits.
