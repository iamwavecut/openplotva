---
last_mapped: 2026-07-05T20:39:15Z
total_files: 428
total_tokens: 1539840
---

# OpenPlotva Codebase Map

> Auto-generated by Cartographer. Last mapped: 2026-07-05.
...
| `memory_runtime.rs` (~5k) | Memory consolidation wiring: extractor composition, daily-run scheduling, claim/process pipeline |
...
No `sqlx`, no `tracing`, no `futures` — policy is pure/sync.
...
flowchart LR
    ...
    Cards --> Resolve[partition Supersede/Competing]
    Resolve --> Write[(memory_cards,<br/>memory_episodes,<br/>memory_links,<br/>memory_runs)]
...
+ pg_trgm. 152 migrations (`0_init` → `151`). Core table groups:
...
- **Memory**: `memory_cards` (bitemporal: `recorded_at`/`retracted_at`, status active/superseded/deleted/competing, `conflict_group`), `memory_episodes`, `memory_sources`, `memory_links`, `memory_runs` (lease-based queue).
...
(transaction-time) → `128` (competing). Routing: `121-125` (declarative v1)
