...
**Request-derived leak guard (2026-09-14).** The marker list could not follow prompt changes, so `openplotva_dialog::ReplyLeakGuard` (`leak_guard.rs`) now fingerprints the request itself: 6-word shingles + snake_case/long tag names of the system messages and the `<chat_context>` message (memory chunks, `<custom_persona>` and `<daily_persona_accent>` use 8-word shingles so short facts and persona catchphrases stay reusable), plus per-entry 8-word shingles, exact text and sender labels of every participant-authored transcript entry (the bot's own past replies only register its speaker label: repeating oneself is the loop problem, not a leak). `finalize_dialog_reply_with_guard(content, &guard)` adds `PromptLeak` (verbatim contract/persona/context copy, a prompt tag, or a snake_case contract identifier such as `base_voice` named in prose — all outside code spans; transcript wrapper names like `message_type` count only in tag form because users may ask about protocol fields) and `TranscriptLeak` (exact echo of one entry, ≥2 entries echoed, or ≥2 `Sender:` labelled lines) on top of the old pipeline; both are retryable in both providers. Replayed on 1,498 random production traces the full guard changed 4 verdicts (all verbatim echoes of a participant message); on 61 traced leaks it suppressed 42 and recovered a clean answer from 14. `openplotva_llm::aifarm::reply_leak_guard(messages, input)` builds it from the exact `ChatMessage`s of the step; aifarm applies it to the final answer, the text beside tool calls and every `send_message` body, gemini to the final answer. Recovery before rejection (both searches skip *closed* code spans via `leak_guard::mask_code_spans`, whose byte-preserving mask keeps offsets valid on the original — a stray opener masks nothing — and both act only on elements that open a line): `<answer>`/`<final_answer>`/`<response>` envelopes yield their text (`answer_envelope` on `SanitizedAssistantText`), block-form `<think>`-style blocks are stripped wherever they sit (an unclosed block-form one is a `ReasoningLeak`, an inline mention is prose, and an orphan close tag that ends its line ends a reasoning run so the reply is what follows), and a JSON document carrying a reply-target key of the rendered history (`reply_to_id`/`reply_to_user`) yields its `answer`/`response`/`text`/`content`/`message`/`reply` string (no text → `ProtocolOnly`; any other JSON, `sender`/`message_id` keys included, is left alone as possibly user-requested). A copied `<message id=…>`/`<last_message>` block ahead of prose is `ContextLeak` unless an answer envelope followed it.
...
- **Busy primary pools are waited for, not skipped** *(2026-09-14)*: a `Role::Primary` candidate whose pool is full is waited for up to `DIALOG_AIFARM_POOL_PRIMARY_CAPACITY_WAIT_MS` (default 12 s; `RoutedAttemptWalker::with_primary_slot_wait`, capped by the caller deadline, charged to `total_slot_wait`, `primary_slot_wait` routing event) before the fallback tail runs; fallback/overflow candidates still skip instantly. Evidence: with the dialog router's old 1 s all-busy wait and instant primary skips, Gemma sat at 0.45–0.59 slot occupancy while ~750 turns/day were shed to the tool-less genkit path; the isolated 3090 replay showed the GPU is prefill-bound (~25 req/min at 11.5k tokens) and that raising vLLM `max-num-seqs` buys +5–7% throughput for ×4–×9 TPOT, so queueing on the primary is the lever, not more sequences.
...
**To change reply finalization / leak handling**: edit `finalize_dialog_reply_with_guard` + the marker/label const arrays in `openplotva-dialog/lib.rs`, or the rules in `openplotva-dialog/leak_guard.rs` — both providers pick it up automatically. Prompt wording changes need no guard update: the guard fingerprints the rendered request. Add leak markers as tag-shape prefixes (not bare words); add reasoning-channel labels with the line-scope guard intact. If a new suppression variant is needed, map it to a retryable error in **both** `aifarm::final_answer_error_from_suppression` and `gemini::gemini_suppression_message`.
