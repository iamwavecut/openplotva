...
pub const HELP_INTRO_MESSAGE_TEMPLATE: &str = r#"<h3>Привет! Я {{.BotName}}</h3>

<p>Я тут для общения, рисования и полезных задач.</p>

<p>Хочешь познакомиться с возможностями? Нажми кнопку ниже.</p>"#;

pub const HELP_REDIRECT_MESSAGE: &str =
    "Чтобы получить справку, откройте личный чат со мной и нажмите кнопку ниже.";

pub const HELP_MESSAGE_TEMPLATE: &str = r#"<h3>Привет! Я {{.BotName}}</h3>

<p>Я тут для общения, рисунков и полезных задач. Я стараюсь быть живой и понятной.</p>

<h3>Как со мной общаться</h3>

<ul>
<li>В личке отвечаю всегда — просто напиши, как человеку.</li>
<li>В группе можно написать «{{.BotName}}, ...» или ответить на любое моё сообщение.</li>
<li>Я могу иногда отвечать случайно на сообщения в группах — это настраивается в <code>/settings</code> (реактивность).</li>
</ul>

<h3>Рисование и правки</h3>

<p>Я могу рисовать по твоим запросам: <code>{{.BotName}}, нарисуй кота в космосе</code></p>

<p><b>Правки:</b> ответь на картинку или пришли новую и напиши, что изменить.</p>

<p><b>Плюс:</b> я улучшаю описания сама, этого не видно. <b>Минус:</b> есть очереди и лимиты, чтобы всем хватало ресурсов.</p>

<p><b>Ограничения:</b> примерно 5/10/20 картинок на пользователя в течение окна в 10/15/30 минут.</p>

<h3>Полезности</h3>

<p>Ищу в интернете, читаю ссылки, кратко пересказываю YouTube, перевожу тексты и показываю курсы валют.</p>

<p>Важный контекст теперь собирается фоном, а не в каждом ответе.</p>

<h3>Пересказы чата</h3>

<p>Можешь попросить: «о чём говорили за сутки», «перескажи последние 6 часов» или «что было в последних 300 сообщениях».</p>

<h3>Роли и стиль</h3>

<p>Иногда я говорю в разных ролях — это ежедневные, уникальные для чата персонажи.</p>

<p>Можно оставить как есть или задать свою персону в <code>/settings</code>.</p>

<h3>VIP</h3>

<p>Если хочется быстрее и больше: приоритетная VIP очередь, в 2 раза больше генераций, лучшее качество и генерация песен по команде <code>/song</code>.</p>

<p>Оформить можно через <code>/vip</code>.</p>

<h3>Быстрый старт</h3>

<p>«Придумай текст для поста», «сделай обложку», «переведи на английский», «расскажи про эту ссылку», «перескажи, что было за день».</p>

<p>Поддержать меня можно через <code>/donate</code>.</p>"#;
...
mod tests {
    ...
    fn intro_plan_matches_go_private_start_help_intro() -> Result<(), Box<dyn std::error::Error>> {
        ...
        assert!(
            plan.message
                .text
                .starts_with("<h3>Привет! Я Пло&lt;тва&gt;</h3>")
        );
        assert!(
            plan.message
                .text
                .contains("</p>\n\n<p>Хочешь познакомиться")
        );
        assert_eq!(
