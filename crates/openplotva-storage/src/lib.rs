...
pub const SQL_LIST_VIP_EVENTS_BY_USER: &str = "SELECT ve.id, ve.user_id, ve.event_type, ve.delta_seconds, ve.effective_expires_at, ve.subscription_id, ve.actor_user_id, actor.username AS actor_username, actor.first_name AS actor_first_name, ve.reason, ve.created_at, s.telegram_payment_charge_id, s.provider_payment_charge_id, s.expires_at AS subscription_expires_at, s.canceled_at AS subscription_canceled_at, s.refunded_at AS subscription_refunded_at FROM vip_events ve LEFT JOIN LATERAL (SELECT username, first_name FROM telegram_users_effective WHERE id = ve.actor_user_id LIMIT 1) actor ON TRUE LEFT JOIN subscriptions s ON s.id = ve.subscription_id WHERE ve.user_id = $1 ORDER BY ve.id DESC";
...
pub const TELEGRAM_FILE_ASR_STATUS_PENDING: &str = "pending";

pub const TELEGRAM_FILE_ASR_STATUS_PROCESSING: &str = "processing";
...
#[derive(Clone, Debug, Default, PartialEq)]
pub struct TelegramFileAsrUpdate {
    /// Telegram stable unique file ID.
    pub file_unique_id: String,
    /// New ASR status.
...
pub struct TelegramFileRecord {
    ...
    /// ASR transcript.
    pub asr_text: Option<String>,
    /// ASR provider/engine that produced the transcript.
    pub asr_provider: Option<String>,
    /// ASR model that produced the transcript.
    pub asr_model: Option<String>,
...
impl PostgresTelegramFileStore {
    ...
    pub async fn update_asr(
        &self,
        params: &TelegramFileAsrUpdate,
    ) -> Result<TelegramFileRecord, StorageError> {
        let row = sqlx::query(SQL_UPDATE_TELEGRAM_FILE_ASR)
            .bind(&params.file_unique_id)
            .bind(&params.asr_status)
            .bind(params.asr_text.as_deref())
...
fn telegram_file_from_row(row: PgRow) -> Result<TelegramFileRecord, StorageError> {
    ...
    Ok(TelegramFileRecord {
        ...
        asr_status: row.try_get("asr_status")?,
        asr_text: row.try_get("asr_text")?,
...
mod tests {
    ...
    fn telegram_file_storage_sql_matches_go_contract() {
        ...
        assert!(
            super::SQL_GET_TELEGRAM_FILE.contains("asr_status")
        ...
        assert!(
            super::SQL_CLAIM_TELEGRAM_FILE_ASR_PROCESSING
                .contains("asr_requested_at < ($2 - INTERVAL '10 minutes')")
        );
    ...
    fn telegram_file_metadata_helpers_match_go_caption_pending_and_ref_rules() {
        ...
        let base = super::TelegramFileRecord {
            ...
            asr_status: super::TELEGRAM_FILE_ASR_STATUS_PENDING.to_owned(),
            asr_text: None,
            asr_provider: None,
            asr_model: None,
            asr_latency_ms: None,
    ...
    fn vip_storage_sql_matches_go_query_contracts() {
        ...
        assert_eq!(
            ...
            "SELECT ve.id, ve.user_id, ve.event_type, ve.delta_seconds, ve.effective_expires_at, ve.subscription_id, ve.actor_user_id, actor.username AS actor_username, actor.first_name AS actor_first_name, ve.reason, ve.created_at, s.telegram_payment_charge_id, s.provider_payment_charge_id, s.expires_at AS subscription_expires_at, s.canceled_at AS subscription_canceled_at, s.refunded_at AS subscription_refunded_at FROM vip_events ve LEFT JOIN LATERAL (SELECT username, first_name FROM telegram_users_effective WHERE id = ve.actor_user_id LIMIT 1) actor ON TRUE LEFT JOIN subscriptions s ON s.id = ve.subscription_id WHERE ve.user_id = $1 ORDER BY ve.id DESC"
        );
        assert_eq!(
    ...
    -> Result<(), Box<dyn Error>> {
        ...
        let result: Result<(), Box<dyn Error>> = async {
            ...
            assert_eq!(
                store
                    .history_message_payloads(chat_id, message_id, range_start, 77, range_start)
                    .await?
                    .len(),
                1
            );
            ...
            assert!(
                store
                    ...
                    .await?
                    .is_empty(),
                "global reset/TTL cutoff must exclude reply ancestors"
            );
            assert!(
                store
                    .history_message_payloads(chat_id, message_id, range_start, 77, range_end)
                    .await?
                    .is_empty(),
                "thread reset cutoff must exclude ancestors in the active thread"
            );
            assert_eq!(
                store
                    .history_message_payloads(chat_id, message_id, range_start, 78, range_end)
                    .await?
                    .len(),
                1,
                "a reset in another topic must not hide the explicit ancestor"
            );
    ...
    -> Result<(), Box<dyn Error>> {
        ...
        let start = end - time::Duration::minutes(15);
