...
out="$here/local/$flow"
mkdir -p "$out"
query="select jsonb_build_object('id', id, 'request', raw_request) from llm_request_events where flow = '$flow' and raw_request is not null and coalesce(is_rollup, false) = false order by id desc limit $count"
ssh -o BatchMode=yes "$PROMPT_EVAL_SSH_TARGET" "$PROMPT_EVAL_PSQL -At -c \"$query\"" |
  python3 -c '
import json, sys
...
for line in sys.stdin:
    if not line.strip():
        continue
    row = json.loads(line)
    event_id, request = row["id"], row["request"]
