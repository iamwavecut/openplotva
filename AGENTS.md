...
- App-level private `/settings` command handling lives in `openplotva-app::settings`: preserve Go's private-only WebApp button behavior, blank-`WEBAPP_URL` no-send path, `settingsWebAppURL` signature URL, text `Откройте настройки бота:`, WebApp button text, reply parameters, immediate queue placement, virtual-message insertion, and queue-carried `BypassChatRestrictions=true`. Group `/settings` remains a separate taskman/fetcher slice.
