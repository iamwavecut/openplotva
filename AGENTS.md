...
- Fetcher-facing inline keyboards should use the `openplotva-telegram` helper wrappers for Go `NewInlineKeyboardButtonData`, `NewInlineKeyboardButtonURL`, `NewInlineKeyboardRow`, and `NewInlineKeyboardMarkup` semantics so callback data, URLs, empty keyboards, and copied rows stay payload-compatible.
