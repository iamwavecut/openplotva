...
- Keep fetcher-style message metadata helpers in `openplotva-updates` until the fetcher route is ported: `resolve_message_sender` mirrors Go `utils.ResolveMessageSender`, `build_message_meta` mirrors Go `buildMessageMeta`, `detect_message_type` prefers trimmed `message`-sourced attachment kinds before Telegram message fields, fallback order is voice, video, audio, document, location/venue, contact, photo/sticker image, text, then `text`, and `collect_media_attachments` reuses the attachment extractor with promoted first-image references and dedupes by the Go attachment key rules.
...
- New inbound history writes should use `openplotva_updates::build_history_text_entry` to produce the Go-shaped text `MessageEntry` payload, then pass its fields to `openplotva_storage::PostgresHistoryStore::upsert_history_entry`: the builder preserves Go text/caption/original-text normalization, sender metadata filling, `msg:<message_id>` entry IDs, user/model role selection from the current bot ID, forwarded/via-bot/thread metadata, and the storage helper preserves Go `UpsertHistoryEntry`, calls `ensure_chat_history_partition($1::date)` before writing, upserts on `(bucket_day, chat_id, entry_id)`, and invalidates `plotva:chat_history_cache:v2:*` after the write.
