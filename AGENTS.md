...
- Fetcher-facing callback data helpers live in `openplotva-telegram`: `parse_callback_action`, `callback_handler_for_action`, `callback_query_route`, `callback_query_ack_request`, `callback_query_ack_method`, `checkin_theme_selection_ack_method`, and the check-in theme callback helpers mirror Go's long/short callback keys, `processCallbackQuery` pre-handler order, legacy/actionless ack split, known handler groups, rate-limit skip, settings callback routing, Go's empty `answerCallbackQuery` terminal acknowledgements, and initiator-only theme-selection alert behavior. Keep `dl_x` inventoried as callback data but unrouted unless the Go handler list changes.
