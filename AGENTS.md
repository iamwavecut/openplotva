...
- When a decoded-update handler is ready to own fetcher-style message handling, wrap it with `openplotva_app::updates::process_update_with_state_and_history_store_at` or `handle_update_with_history` instead of duplicating storage calls. These helpers keep history failures non-fatal and logged, while handler failures still fail the handle stage; they are opt-in and must not be used to install a placeholder/no-op consumer.
