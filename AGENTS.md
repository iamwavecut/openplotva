...
- New inbound history writes should use `openplotva_storage::PostgresHistoryStore::upsert_history_entry` with a Go-shaped `MessageEntry` JSON payload: it preserves Go `UpsertHistoryEntry`, calls `ensure_chat_history_partition($1::date)` before writing, upserts on `(bucket_day, chat_id, entry_id)`, and invalidates `plotva:chat_history_cache:v2:*` after the write.
