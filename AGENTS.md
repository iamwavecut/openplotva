...
- Keep Telegram update startup methods and sources in `openplotva-telegram`: use `build_get_updates_method`, `build_set_webhook_method`, `build_delete_webhook_method`, `LongPollUpdateSource`, and `TELEGRAM_WEBHOOK_PATH` for concrete `carapax` source wiring instead of reconstructing allowed updates, offsets, retries, or timeouts at the app layer.
- The Rust app shell starts the long-poll update producer when `OPENPLOTVA_CONNECT_SERVICES=true` and `BOT_KEY` is set: the worker deletes the webhook first like Go, then feeds `LongPollUpdateSource` into `plotva:updates:queue`. A `deleteWebhook` failure is logged inside the worker and must not fail app startup, matching Go's background `StartBot` goroutine.
