...
- After completing local build or container smokes, remove task-owned disposable containers, images, volumes, and BuildKit cache. Never prune shared or user-owned caches indiscriminately.
...
The default path for delivering every feature or fix into the repository:

- **Branch + ready PR.** Ship each change on its own `feat/...` or `fix/...` branch and open a ready-for-review PR into `main`. Do not create draft PRs by default: draft PRs skip the full PR-Agent review path in this repository. Use a draft PR only when the user explicitly asks for one or when you intentionally need to prevent automated review before the branch is ready. Never commit straight to `main`, and never self-attribute the work to Claude or any AI in branch names, commit messages, or PR text.
...
- **Open the PR, then start the PR delivery loop.** After the feature branch is pushed and the PR exists, monitor it on a ~60s cadence until it is mergeable. Every iteration must poll both CI/check status and review artifacts: issue comments, review summaries, review decisions, and inline review comments/threads.
- **Treat review artifacts as work, not notifications.** When a new or edited comment, review, or inline thread appears, read the full latest body before acting. If it is valid, make the smallest necessary code/doc/test change, rerun the relevant local checks, commit, push, and restart the loop. If it is wrong, harmful, already addressed, or low-value churn, reply with the concrete reason. Inline review threads must be replied to and resolved; non-thread comments get a direct explanatory reply when needed.
- **Do not let metadata edits interrupt review.** Editing a PR body/title, replying to Danger, or other metadata/comment maintenance must never cancel an in-progress PR-Agent/Qodo review. The PR automation workflow must not run on `pull_request.edited`; if a new review is needed, trigger it explicitly with `workflow_dispatch` instead of relying on an edit event. If PR-Agent was cancelled before publishing its review/suggestions, rerun that workflow and wait for it before merge.
