...
- Payment SQL row storage belongs in `openplotva-storage::PostgresPaymentStore`: preserve Go `subscriptions`/`donations` SQL, `telegram_payment_charge_id` conflict behavior, active-subscription filtering that excludes canceled/refunded/admin-grant rows, first-write-wins cancel/refund timestamps, and donation duplicate handling through no-row results. VIP ledger SQL storage belongs in `openplotva-storage::PostgresVipStore`: preserve Go `vip_cache` queries, `vip_create_event` function calls, event idempotence for subscription-scoped payment/refund events, summary/list row shapes, actor joins, subscription joins, and cleanup of expired cache rows. Successful-payment orchestration belongs in `openplotva-app::payments`: preserve Go `XTR` filtering, payload-prefix routing, user persistence as non-fatal, duplicate no-row fallback lookups, subscription VIP ledger event creation, effective-expiry success text, donation thank-you text, and VIP cache invalidation. VIP/donation invoice control-job execution also belongs in `openplotva-app::payments`: use injected effects around `createInvoiceLink`, Go HTML button text, missing-user/error-message behavior, and returned-link validation; the concrete `TelegramClient` effects should build direct `carapax` methods while keeping the pure executor independently testable. Payment taskman dispatch belongs in this same app module: keep `vip_invoice`, `donate_invoice`, and `successful_payment` routing in `execute_payment_control_job_at`, and keep non-payment control kinds out of the payment dispatcher. Go-shaped payment update queueing also lives here: `payment_invoice_control_job_from_update_at`, `enqueue_payment_invoice_command_update_or_else_at`, `successful_payment_control_job_from_update_at`, and `enqueue_successful_payment_update_or_else_at` build and assign high-priority `"vip invoice"`, `"donate invoice"`, and `"successful payment"` jobs to the `control` queue. Payment control-job worker code also lives here: decode queued `StatelessJobItem` payloads through `openplotva-taskman`, execute payment-owned jobs, complete missing-user/successful-payment outcomes that Go treats as non-fatal, and mark unsupported/payment-invoice execution errors failed like Go taskman executor errors. The payment-aware decoded-update wrapper `handle_payment_update_or_else_at` composes Go pre-checkout acknowledgement, successful-payment control-job queueing, invoice-command control-job queueing, and final delegation to the future real fetcher route; do not replace it with inline successful-payment processing once taskman is active. Non-private `/vip` and `/donate` redirect builders live here too: preserve Go deep-link shapes, button text, reply parameters, no parse mode, VIP benefits text, donation benefit text, valid donation amount forwarding, and invalid donation amount fallback to `start=donate`. Existing-VIP status lookup and message builders also live here: preserve Go `buildVIPStatusView` order (active VIP summary, active subscription for the cancel button, then external `vip_cache`), active-ledger status text, external-cache status text, `vip.DisplayDaysLeft` rounding, HTML parse mode, reply parameters with `allow_sending_without_reply=true`, and the `{"action":"cancel_vip"}` button only when an active subscription row exists. The invoice-command path reports non-private redirects, missing VIP users, and invalid donation amounts so the caller can send the exact Go side effects. Keep Telegram SDK types, admin command text, direct Bot API request builders, and taskman/control-job scheduling outside the storage stores.
