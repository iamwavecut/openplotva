...
- Keep fetcher-style message metadata helpers in `openplotva-updates` until the fetcher route is ported: `detect_message_type` prefers trimmed `message`-sourced attachment kinds before Telegram message fields; fallback order is voice, video, audio, document, location/venue, contact, photo/sticker image, text, then `text`; `collect_media_attachments` reuses the attachment extractor with promoted first-image references and dedupes by the Go attachment key rules.
