...
- Keep fetcher-style message metadata helpers in `openplotva-updates` until the fetcher route is ported: `fetcher_message_text` mirrors Go `Fetcher.getTextFromMessage` text/caption/sticker/audio/video/document/contact fallback behavior, `resolve_message_sender` mirrors Go `utils.ResolveMessageSender`, `build_message_meta` mirrors Go `buildMessageMeta`, `detect_message_type` prefers trimmed `message`-sourced attachment kinds before Telegram message fields, fallback order is voice, video, audio, document, location/venue, contact, photo/sticker image, text, then `text`, and `collect_media_attachments` reuses the attachment extractor with promoted first-image references and dedupes by the Go attachment key rules.
...
- New inbound history writes should use app-level `openplotva_app::updates::persist_inbound_message_history`, which calls `openplotva_updates::build_history_text_entry` and the `InboundHistoryStore` adapter for `openplotva_storage::PostgresHistoryStore`. This preserves Go text/caption/original-text normalization, sender metadata filling, `msg:<message_id>` entry IDs, user/model role selection from the current bot ID, forwarded/via-bot/thread metadata, Go `UpsertHistoryEntry`, `ensure_chat_history_partition($1::date)`, upserts on `(bucket_day, chat_id, entry_id)`, and `plotva:chat_history_cache:v2:*` invalidation.
- Edited inbound message history writes should use app-level `openplotva_app::updates::persist_edited_message_history`, which accepts one decoded `edited_message` update, extracts Go fetcher-style fallback text, and calls `openplotva_storage::PostgresHistoryStore::update_message_entry` through `EditedHistoryStore`. This preserves Go `Fetcher.processEditedMessage`/`history.Service.UpdateMessage` semantics: update an existing text entry only, normalize text/original-text/meta attachments, avoid creating missing history rows, and invalidate `plotva:chat_history_cache:v2:*`.
