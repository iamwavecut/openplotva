...
- Chat permission policy primitives live in `openplotva-server`, with Telegram-free `ChatSettings`/`ChatSettingsUpdate` models in `openplotva-core` and SQLx chat-settings helpers in `openplotva-storage`. Runtime send gating and permission-error auto-disable wiring still need to be connected through the outbound dispatcher/app layer before marking permissions complete.
