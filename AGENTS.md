...
- App-level virtual-message producers live in `openplotva-app::virtual_messages`. Text and sticker sends insert unresolved `message_id_map` rows before dispatcher enqueue, keep insert failures non-fatal like Go logs, use Go-compatible fingerprints, respect immediate queue placement, and let `send_work_item_and_resolve` resolve mappings from successful Telegram `Message` responses. Preserve this pattern for future photo/audio/media producers.
- Redis-backed chat rate-limit expiry storage belongs in `openplotva-storage::RedisRateLimitStore`. Preserve the Go key prefix `plotva:rate_limited_chat:`, Redis TTL behavior, and strict `now < expiry` active boundary. The storage codec intentionally uses the approved Rust-native JSON timestamp value instead of Go gob; app-level policy wiring should treat missing or unreadable Redis values as not rate-limited like Go `IsRateLimited`.
