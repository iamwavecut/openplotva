...
## Batch Execution Preference

- Prefer batched progress over narrow step-by-step slices when evidence is strong enough: edit multiple files together, port adjacent producer/executor/storage/doc pieces in one pass, and pursue independent targets in parallel where that reduces total time.
- Make broad global changes when they are the optimal path to the real implementation goal, while still preserving the reference snapshot, Go-repo read-only rule, user-facing contract, and verification gates.
- If generic workflow skills or local habits push toward overly serial execution, this repo favors larger coherent batches and faster end-to-end movement.

## Verification Batching

- Prefer targeted tests for the changed slice during active development, then run broader gates after several related slices or before a meaningful push/release checkpoint.
- Do not run full `cargo test --workspace`, full clippy, and repeated package tests after every small edit unless the touched area is cross-cutting or already suspicious.
- Batch verification work the same way code work is batched: one narrow signal per slice, one broader signal per batch, and full gates only when the batch is worth sealing.
- When useful, run targeted checks in background terminal sessions while continuing with the next independent slice. Harvest and fix failures in batches instead of blocking every edit cycle on one test command.
...
- App-level callback query pre-handler behavior lives in `openplotva-app::callbacks`: preserve Go `processCallbackQuery` terminal routes by acknowledging orphan/empty/legacy/actionless/unknown callbacks, skipping rate-limited chats without acknowledgement, sending the cached settings callback acknowledgement, and delegating known handler groups exactly once to the future fetcher route. Concrete delete/VIP/check-in/delete-drawing/delete-lyrics callback side effects stay with their owning feature slices.
- App-level accepted-but-unprocessed update handling lives in `openplotva-app::skipped`: `SkippedUpdateHandler` consumes Go-allowed `chosen_inline_result` and `chat_join_request` updates after state extraction because Go has no processors for them and only logs `Skipping message type`. Keep this wrapper near the end of the future update-handler chain, before any terminal error/unknown handler, so these updates do not leak into message/guest/payment routes.
...
- App-level inline query handling lives in `openplotva-app::inline`: preserve Go `processInlineQuery` by answering each inline query directly with one personal cached article titled `Шевелись, Плотва!`, using the inline query ID as the result ID, the query text as message text, `cache_time=1`, and no dispatcher queue or virtual-message mapping. Telegram answer failures are logged and consume the update like Go `tryRequest`.
...
- App-level member service/state behavior lives in `openplotva-app::members`: preserve Go `processLeftChatMember` and `updateMemberState` for left-member service messages plus `chat_member`/`my_chat_member` state updates, including delete-before-delegate semantics, active/departed bot communication toggles, Go `chat_member_sync`/`chat_admins_sync` control-job payloads, and Postgres `chat_members` delete/upsert behavior. These handlers delegate after side effects like the Go state/service-event paths and must not be installed with a no-op downstream consumer.
...
- Keep fetcher-style message routing helpers in `openplotva-updates` until the fetcher route is ported: `parse_if_addressed` mirrors Go `parseIfAdressed` including bot first-name/transliteration stripping, group mention stripping, private-chat addressing, reply-to-bot addressing, and forum topic-root reply suppression; `is_settings_command_message` mirrors Go's `/settings` command target rules and must stay aligned with the permission/block bypass path; `parse_edit_command` mirrors Go `parseEditCommand` edit-verb detection for image-edit routing and caption fallback; `resolve_draw_prompt_from_message` and `draw_prompt_with_reply_context` mirror Go draw reply-context prompt handling and intentionally use `ReplyToMessage.Text`, not media/caption fallback text; `compose_image_prompt` and `edited_image_prompt_update` mirror Go pending-image-job prompt updates by appending unique trimmed vision/attachment context after the base prompt; `should_handle_addressed_message` and `should_handle_random_response` mirror Go bot-loop and captionless-media random-response gates; `react_message_words` mirrors Go `React` command-word splitting and first-word fallback; guest helpers `guest_visible_text`, `strip_guest_address_prefix`, `guest_current_request_text`, `guest_request_has_visible_text`, `guest_has_other_bot_mention`, `guest_message_reject_reason`, `is_guest_unsupported_feature_request`, `normalize_guest_command_word`, `looks_like_guest_history_summary_request`, `GuestChainMessage`, `format_guest_chain_for_prompt`, `build_guest_dialog_text`, and `build_guest_shield_query_text` mirror Go guest-message visible-text, address stripping, reply fallback, guard-order semantics, add-to-chat unsupported-feature gating, guest-chain prompt formatting, dialog text shaping, and Shield query text/truncation.
- App-level guest-message routing lives in `openplotva-app::guest`: `GuestMessageUpdateHandler` consumes `guest_message` updates, applies Go guard order, answers unsupported feature requests with the add-to-chat guest HTML response, builds guest dialog/shield/chain inputs through injected effects, falls back to Go guest timeout HTML on empty or failed dialog output, and remembers the guest chain only after a successful answer send. `GuestChainCache` mirrors Go's ephemeral in-memory chain cache with 24 hour TTL, 15-message cap, reply-answer SHA1 keying, HTML stripping/whitespace compaction, stale-message trimming, and role default names. Concrete dialog and Shield effects are still separate vertical slices; do not wire this wrapper with placeholder effects that pretend full guest runtime is complete.
- App-level `/settings` command handling lives in `openplotva-app::settings`: preserve Go's private WebApp button behavior, blank-`WEBAPP_URL` no-send path, `settingsWebAppURL` signature URL, text `Откройте настройки бота:`, WebApp button text, reply parameters, immediate queue placement, virtual-message insertion, and queue-carried `BypassChatRestrictions=true`. Group `/settings` builds the high-priority `"group settings"` control job for the `control` queue, preserves Go caller/sender guards, sends the exact immediate decline/wait/failure notices, and executes the group-settings control job through injectable permission/admin-sync effects. `SettingsUpdateHandler` composes private settings, group settings, and new-member follow-up producers before delegating to the future real fetcher route; it must not be installed with a no-op downstream consumer that drains production updates. The executor sends Go's permission-check failure text, no-rights text, and private-chat deep-link button text/URL with `BypassChatRestrictions=true`; it intentionally omits topic `message_thread_id` when reconstructing Go `buildControlMessage`. The admin-only `/admin_chat_settings` command path is ported after admin authorization: normal error/usage replies stay non-bypassed, resolved target buttons use Go settings WebApp URLs, immediate queue placement, and queue-carried `BypassChatRestrictions=true`, and concrete target resolution uses `carapax` `getChat` with Go-shaped numeric/username error behavior. Concrete `canOpenGroupSettings` storage/Telegram lookup is ported through `PostgresChatMemberStore`, `carapax` `getChatMember`, and Rust conversion from `carapax::types::ChatMember` to Go-shaped upsert params; `carapax` Bot API 10 admin variants do not expose legacy aggregate send permissions, so send-permission upserts are populated from restricted-member variants where the API exposes them. Concrete `syncChatAdmins` API/DB fallback is ported through `carapax` `getChatAdministrators`, Go-shaped admin membership upserts, stored-admin fallback, admin user persistence, and best-effort Redis admin-ID cache writes under `chat:{id}:admins` with a 30 minute TTL. Ephemeral wait/failure notices carry one-minute delete timing through dispatcher persistence and are tracked in Redis after successful first-message sends; unsupported-sender declines and private settings remain non-ephemeral like Go. The Redis key/codec/TTL/filter storage surface and runtime cleanup worker are ported: cleanup scans `ephemeral_messages:*`, applies the strict Go expiry boundary, deletes Telegram messages in batches of 10, and removes store records after delete attempts.
- New-member follow-up producer and control-job execution also live in `openplotva-app::settings`: preserve Go `processNewChatMembers` by best-effort upserting each new member as `member` with non-anonymous/non-editable flags, assigning the high-priority `"new members followup"` control job, detecting `bot_was_added` from the configured bot ID, preserving new-member ID order, and sending the same one-minute queue-failure notice. Preserve Go `ControlKindNewMembersFollow` execution by enabling chat communication only when `bot_was_added=true`, checking whether the chat remains blocked, queuing the exact immediate `BypassChatRestrictions=true` unblock notice with the `⚙️ Настройки` URL button, preserving the empty URL if the bot username is unavailable, and always attempting the greeting follow-up boundary. Full greeting debounce/composition/storage is still future fetcher work.
...
- App-level message activity tracking lives in `openplotva-app::activity`: `MessageActivityUpdateHandler` mirrors Go `trackMessageSenderActivity` before downstream routing, tracks only non-bot user senders, updates `chat_members.last_message_at`, writes `chat_active_users`, and on active-user write failure persists the Telegram user then retries the active-user write. The SQL helpers live on `openplotva-storage::PostgresChatMemberStore` and preserve Go `UpdateMemberLastMessage`, `UpsertChatActiveUser`, `ListActiveParticipants`, and `ListActiveParticipantsFromTable` query shapes.
...
- App-level edited-message side effects live in `openplotva-app::edited`: after `UpdateHandlerWithHistory` has handled Go `history.UpdateMessage`, `EditedMessageUpdateHandler` consumes `edited_message` updates, derives Go `newMessageContext`/`parseIfAdressed` fields, updates dialog debounce entries, updates pending dialog jobs, and updates pending image jobs through injected effects. These effect failures are logged/suppressed like Go; the real taskman/dialog-debounce implementation is still a later taskman slice. Keep this handler in front of the future full message fetcher route so edited messages are not processed as fresh messages.
